...
corrupcion = import("https://raw.githubusercontent.com/CarlosChavarri23/TRABAJO-GRUPAL-POL304/main/Corrupcion-2019.csv")
...
```{r}
str(data_hector)
as.numeric(data_hector$`2019`)
as.numeric(data_hector$`P3: Human Rights`)
...
```{r}
str(data_hector)
data_hector$` CORRUPCTION`=as.numeric(data_hector$` CORRUPCTION`)
data_hector$` HUMANRIGTHS`=as.numeric(data_hector$` HUMANRIGTHS`)
data_hector$PEACEINDEX=as.numeric(data_hector$PEACEINDEX)
```
...
export(dataFINAL,"BASEGRUPAL.csv")
```
